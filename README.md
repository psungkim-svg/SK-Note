...
- **화면 과다 확대 수정**: 입력창을 누를 때 폰이 자동으로 확대하던 것을 차단 (maximum-scale=1)
- **노트 작성 화면 진짜 전체화면**: 구형 폰 브라우저에서도 화면 100% 채움 (position:fixed 방식)
- **오류 표시 장치**: 무언가 실패하면 화면에 ⚠️ 메시지가 뜸 (문제 파악용)
- 아이콘 노란색 배경 적용
